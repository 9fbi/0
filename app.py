...
st.set_page_config(
    page_title="Cyber Threat Dashboard",import streamlit as st
import pandas as pd
import numpy as np
import streamlit.components.v1 as components
...
h1, h2, h3 { text-shadow: 0 0 10px #00ff00; }
...
def generate_data():
    ...
    countries = [
        ...
        "Brazil","Canada","Australia","South Korea","Israel","Italy",
        "Spain","Netherlands","Singapore","UAE","Turkey","South Africa"
    ]
    return pd.DataFrame({
...
coords = {
    ...
    "Germany": (51.16,10.45), "Russia": (61.52,105.31), "UK": (55.37,-3.43),
    "France": (46.22,2.21), "Japan": (36.20,138.25), "Brazil": (-14.23,-51.92),
    "Canada": (56.13,-106.34), "Australia": (-25.27,133.77), "South Korea": (35.90,127.76),
    "Israel": (31.05,34.85), "Italy": (41.87,12.57), "Spain": (40.46,-3.75),
    "Netherlands": (52.13,5.29), "Singapore": (1.35,103.82), "UAE": (23.42,53.85),
    "Turkey": (38.96,35.24), "South Africa": (-30.56,22.93)
}
...
live = live.sort_values("Attacks", ascending=False).head(8)
...
map_data = live.copy()
map_data["lat"] = map_data["Country"].map(lambda x: coords[x][0])
map_data["lon"] = map_data["Country"].map(lambda x: coords[x][1])
...
st.download_button("⬇ Download Full Dataset", csv, "cyber_threat_data.csv", "text/csv")
    page_icon="🛡️",
    layout="wide"
)
